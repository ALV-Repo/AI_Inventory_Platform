...
type PO = { id: number; po_number: string; supplier_id: number };
...
export default function PurchaseReturnsPage() {
  ...
  async function load() {
    try {
      setLoading(true);
      setError("");
      const [ret, pos] = await Promise.all([
        api.request<PurchaseReturn[]>("/purchases/returns"),
  ...
  async function createReturn() {
    if (!form.purchase_order_id || !form.product_id || !form.quantity) return;
    try {
      setSaving(true);
      await api.request("/purchases/returns", {
        method: "POST",
        body: JSON.stringify({
      ...
      setShowCreate(false);
      setForm({ purchase_order_id: "", supplier_id: "", warehouse_id: "1", reason: "", product_id: "", quantity: "", unit_price: "" });
      await load();
    } catch (e: unknown) {
  ...
  const totalValue = returns.reduce((a, r) => a + (r.total_amount || 0), 0);
      ...
      <div className="mx-auto max-w-5xl px-6 py-8">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">Return goods to suppliers</p>
          </div>
        ...
        <div className="mb-6 grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Total Value", value: inr(totalValue), color: "text-red-600" },
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
          {loading ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-sm text-gray-500">No purchase returns yet</p>
            </div>
          ) : (
            <table className="w-full text-left">
              <thead className="border-b border-gray-200 bg-gray-50">
                <tr>
                  {["Return #", "PO Reference", "Reason", "Total", "Status", "Date"].map(h => (
              ...
              <tbody className="divide-y divide-gray-100">
                {returns.map(r => (
                  <tr key={r.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 font-mono text-sm font-bold text-blue-600">{r.return_number}</td>
                    <td className="px-6 py-4 text-sm text-gray-600">PO-{r.purchase_order_id}</td>
                    <td className="px-6 py-4 text-sm text-gray-600 max-w-[200px] truncate">{r.reason || "-"}</td>
                    <td className="px-6 py-4 text-sm font-bold text-red-600">{inr(r.total_amount)}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${statusColor(r.status)}`}>{r.status}</span>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-500">{fmtDate(r.created_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {showCreate && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" onClick={() => setShowCreate(false)}>
            <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl" onClick={e => e.stopPropagation()}>
              <h2 className="mb-4 text-xl font-bold text-gray-900">New Purchase Return</h2>
              <div className="space-y-3">
                <div>
                  <label className="mb-1 block text-sm font-medium text-gray-700">Purchase Order *</label>
                  <select value={form.purchase_order_id}
                    onChange={e => {
                      const po = orders.find(p => String(p.id) === e.target.value);
                      setForm(f => ({ ...f, purchase_order_id: e.target.value, supplier_id: String(po?.supplier_id ?? "") }));
                    }}
                    className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-500">
                    <option value="">Select PO...</option>
                    {orders.map(po => <option key={po.id} value={po.id}>PO #{po.id}</option>)}
                  </select>
                </div>
                {[
